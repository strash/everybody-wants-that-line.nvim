...
local function get_group_name(group_name)
    ...
    local prefix_len = #Window.prefix
    return Window.prefix .. group_name:sub(prefix_len + 1, prefix_len + 2) .. "Nc" .. group_name:sub(prefix_len + 3)
end
...
local function highlight_text(text, group_name, is_exact_group_name)
    is_exact_group_name = is_exact_group_name or false
...
local function set_color_group_names()
    for k, _ in pairs(cache) do
        Colors.group_names[k] = Window.prefix .. String.pascalcase(k)
        Colors.group_names[k .. "_bold"] = Window.prefix .. String.pascalcase(k .. "_bold")
    end
end
...
local function set_hl_group(group_name, fg_hex, cterm)
    ...
    vim.cmd("hi " .. group_name .. String.cterm(cterm) .. "guifg=#" .. fg_hex .. " guibg=#" .. bg)
end

---Sets hightlight groups
local function set_hl_groups()
    for k, v in pairs(Colors.group_names) do
...
Colors.highlight_text = highlight_text
